...
from .models import  Wallet, PaymentCard
...
def payments(request):
    ...
    if request.method == "POST":
        ...
        payment_id = str(random.randrange(1<<50))
        payment_id = payment_id.zfill(16)
        payment = Payment(
            user = request.user,
...
def addMoneyView(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = request.user
            card = get_object_or_404(PaymentCard, card_number=card_number)
            if card.used:
                messages.error(request, "This Card is already used.")
        ...
        form = AddMoneyForm()
    return render(request, 'orders/add_money.html', {'form': form})